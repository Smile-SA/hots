...
def assign_container_node(node_id, container_id, instance):
    ...
    old_id = instance.df_containers.loc[instance.df_containers['container_id'] == container_id].machine_id.to_numpy()[
        0]

    instance.df_nodes.loc[instance.df_nodes['machine_id'] == node_id, ['cpu', 'mem']] = instance.df_nodes.loc[instance.df_nodes['machine_id'] == node_id, [
        'cpu', 'mem']].to_numpy() + instance.df_containers.loc[instance.df_containers['container_id'] == container_id, ['cpu', 'mem']].to_numpy()

    instance.df_nodes.loc[instance.df_nodes['machine_id'] == old_id, ['cpu', 'mem']] = instance.df_nodes.loc[instance.df_nodes['machine_id'] == old_id, [
        'cpu', 'mem']].to_numpy() - instance.df_containers.loc[instance.df_containers['container_id'] == container_id, ['cpu', 'mem']].to_numpy()

    instance.df_containers.loc[instance.df_containers['container_id'] ==
                               container_id, ['machine_id']] = node_id


def allocation_distant_pairwise(instance, cluster_var_matrix, labels_, lb=0.0):
    """
    First placement heuristic implemented : take two most distant clusters (from their mean profile), and assign by pair (one container from each cluster) on the first available node, and so on. Take the following node if current one has not enough resources.
    """
    ...
    max_nodes = instance.df_nodes_meta.machine_id.nunique()
    ...
    while not stop:
        ...
        if (c_it == 0):
            ...
            for it in list_containers:
                cons_c = instance.df_containers.loc[instance.df_containers['container_id'] == it]['cpu'].to_numpy()[
                    :total_time]
                cap_node = instance.df_nodes_meta.loc[
                    instance.df_nodes_meta['machine_id'] == instance.dict_id_n[n]]['cpu'].to_numpy()[0]
                done = False
                while not done:
            ...
            for it in range(min(len(list_containers_i), len(list_containers_j))):
                cons_i = instance.df_containers.loc[instance.df_containers['container_id'] == list_containers_i[it]]['cpu'].to_numpy()[
                    :total_time]
                cons_j = instance.df_containers.loc[instance.df_containers['container_id'] == list_containers_j[it]]['cpu'].to_numpy()[
                    :total_time]
            ...
            if not (len(list_containers_i) == len(list_containers_j)):
                ...
                if it < len(list_containers_j):
                    for it in range(it, len(list_containers_j)):
                        cons_c = instance.df_containers.loc[instance.df_containers['container_id'] == list_containers_j[it]]['cpu'].to_numpy()[
                            :total_time]
                    ...
                    for it in range(it, len(list_containers_i)):
                        cons_c = instance.df_containers.loc[instance.df_containers['container_id'] == list_containers_i[it]]['cpu'].to_numpy()[
                            :total_time]
...
def allocation_ffd(instance, cluster_vars, cluster_var_matrix, labels_, bound_new_node=50):
    """
    Second placement heuristic, based on "first-fit decreasing" bin-packing heuristic : order clusters by decreasing variance, place all containers belonging to the clusters in this order.
    For each container, try to place it on a node decreasing the node's variance. If not possible, place it on the node whose variance increases the least.
    """

    # TODO add open-nodes system (to run through open nodes only, and open a new one when needed / with criterion)
    ...
    conso_nodes, cluster_done = place_opposite_clusters(instance, cluster_vars,
    ...
    for i in range(instance.nb_clusters):
        ...
        if not cluster_done[idx_cluster_vars[i]]:
            ...
            for container in list_containers:
                ...
                consu_cont = instance.df_containers.loc[instance.df_containers['container_id'] == container]['cpu'].to_numpy()[
                    :total_time]
                idx_node = 0
                min_var = math.inf
                idx_min_var = -1
                ...
                for idx_node in idx_nodes_vars:
                    ...
                    cap_node = instance.df_nodes_meta.loc[
                        instance.df_nodes_meta['machine_id'] == instance.dict_id_n[idx_node]]['cpu'].to_numpy()[0]
                    new_var = (
                        conso_nodes[idx_node] + consu_cont).var()

                    # here we drop the node variance => ok
                    if (new_var <= nodes_vars[idx_node]) and np.all(np.less((conso_nodes[idx_node] + consu_cont), cap_node)):

                        assign_container = idx_node + 1
                        break

                    # we are looking for the node which variance grows the least
                    else:
                        if (new_var < min_var) and np.all(np.less((conso_nodes[idx_node] + consu_cont), cap_node)):
                            min_var = new_var
...
def allocation_spread(instance, nodes_available):
    ...
    for service in instance.services_:
        ...
        while not assign:
            if not enough_resource(nodes_available[assign_s, :, :], service, instance.nodes_init[assign_s].ram, instance.nodes_init[assign_s].cpu):
...
def place_opposite_clusters(instance, cluster_vars, cluster_var_matrix, labels_, min_nodes, conso_nodes):
    total_time = instance.sep_time
    lb = 0.0
