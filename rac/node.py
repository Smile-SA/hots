...
def plot_specificData_allNodes(df_nodes, dataName):
    ...
    pvt.plot(ax=ax)
    fig.suptitle('%s use on all nodes' % dataName)
    plt.draw()


def plot_allData_allNodes_end(df_nodes, total_time):
    fig = plt.figure()
...
def get_list_mean(df_nodes, total_time):
    """
...
def get_list_var(df_nodes, total_time):
    """
...
def print_vmr(df_nodes, total_time,  part):
    """
    ...
    for metric in metrics:
        ...
        print("Global mean : ", float(
            (global_mean/df_nodes.machine_id.nunique())))
        print("Global var : ", float(global_var/df_nodes.machine_id.nunique()))
        print("Global vmr : ", float(global_vmr/df_nodes.machine_id.nunique()))


def get_list_vmr(df_nodes, total_time):
    """
...
def build_dict_id_nodes(df_nodes):
    dict_id_n = {}
    i = 0
...
def plot_allData_allNodes(df_nodes):
    """
